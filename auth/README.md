...
└─ src/
   ...
   ├─ prisma/prisma.service.ts       # PrismaClient wrapper, onModuleInit connect
...
- **Unit** (`pnpm test`): 15 tests — services, controllers, guards with mocked Prisma/UserService.
...
- `package.json#prisma` seed config is deprecated in Prisma 7 (works fine on Prisma 6); migrate to `prisma.config.ts` when upgrading.
